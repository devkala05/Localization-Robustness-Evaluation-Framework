...
echo "[benchmark] dataset=${DATASET_DISPLAY:-UrbanNav} mode=summary algo=${ALGO_DISPLAY} key=${ALGO_ID} case=per_${PER}"
...
bash -lc "${SETUP} && ${ROS_ENV} && ${ALGO_ADAPTER_LAUNCH} run_id:=${PER} perturbation_config:=${CONFIG_PATH} native_lidar_topic:=${ALGO_NATIVE_LIDAR_TOPIC} native_lidar_frame_id:=${DATASET_LIDAR_FRAME:-velodyne} native_imu_topic:=${ALGO_NATIVE_IMU_TOPIC} native_imu_frame_id:=${DATASET_IMU_FRAME:-body} native_camera_topic:=${ALGO_NATIVE_CAMERA_TOPIC} camera_frame_id:=${DATASET_CAMERA_FRAME:-camera_right} point_time_scale:=${ALGO_POINT_TIME_SCALE} point_time_field:=${DATASET_POINT_TIME_FIELD:-time} point_time_unit:=${DATASET_POINT_TIME_UNIT:-auto} ${ALGO_ADAPTER_ARGS}" &
PIDS+=("$!")
...
bash -lc "${SETUP} && ${ROS_ENV} && rosrun fast_lio_urbannav ground_truth_path_node.py _ground_truth_path:=${GT_PATH} _topic:=/ground_truth_path _odom_topic:=/ground_truth_odometry _frame_id:=${DATASET_WORLD_FRAME:-camera_init} _publish_rate:=10.0 _yaw_offset_deg:=${GT_YAW_OFFSET_DEG:-0.0} _publish_full_path:=true" &
PIDS+=("$!")

bash -lc "${SETUP} && ${ROS_ENV} && rosrun localization_benchmark bag_clock_marker.py _frame_id:=${DATASET_WORLD_FRAME:-camera_init}" &
PIDS+=("$!")

if [ -f "${BASELINE_CSV}" ]; then
    bash -lc "${SETUP} && ${ROS_ENV} && rosrun localization_benchmark path_from_csv.py _csv_path:=${BASELINE_CSV} _topic:=/benchmark/baseline_path _frame_id:=${DATASET_WORLD_FRAME:-camera_init}" &
    PIDS+=("$!")
fi
...
echo "[benchmark] playing_rosbag dataset=${DATASET_ID} bag=${BAG_PATH} rate=${BAG_RATE:-0.35}"
bash -lc "${SETUP} && ${ROS_ENV} && rosbag play --clock --rate ${BAG_RATE:-0.35} ${BAG_DURATION_ARGS[*]} ${BAG_PATH} --topics ${ALGO_BAG_TOPICS:-${DATASET_BAG_TOPICS}}"
