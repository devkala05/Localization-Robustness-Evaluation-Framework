...
echo "[run] dataset=${DATASET_ID:-urbannav} algorithm=${DISPLAY_NAME} key=${ALGO} case=${MODE}${PER:+:per_${PER}} gps=${GPS_ENABLE:-off} eval=${EVALUATE}"
...
exec docker run "${TTY_ARGS[@]}" \
  ...
  -e DATASET_ID="${DATASET_ID:-urbannav}" -e DATASET_DISPLAY="${DATASET_DISPLAY:-UrbanNav}" -e DATASET_LABEL="${DATASET_LABEL:-urbannav_hk_tst_20210517}" \
  ...
  -e DATASET_SOURCE_LIDAR_TOPIC="${DATASET_SOURCE_LIDAR_TOPIC:-/velodyne_points}" -e DATASET_SOURCE_IMU_TOPIC="${DATASET_SOURCE_IMU_TOPIC:-/imu/data}" \
  -e DATASET_SOURCE_CAMERA_TOPIC="${DATASET_SOURCE_CAMERA_TOPIC:-/zed2/camera/right/image_raw}" -e DATASET_SOURCE_LEFT_CAMERA_TOPIC="${DATASET_SOURCE_LEFT_CAMERA_TOPIC:-/zed2/camera/left/image_raw}" \
  ...
  -e DATASET_USE_CLOCK_STAMP="${DATASET_USE_CLOCK_STAMP:-false}" \
  -e DATASET_PERTURBATIONS_DIR="${DATASET_PERTURBATIONS_DIR:-/root/catkin_ws/src/localization_benchmark/config/perturbations}" -e DATASET_SEGMENTS_YAML="${DATASET_SEGMENTS_YAML:-/root/catkin_ws/src/localization_benchmark/config/road_segments.yaml}" \
