...
tmux send-keys -t "${BAG_PANE}" "${SETUP} && ${ROS_ENV} && echo 'Waiting 8s for bridge, adapter, FAST-LIO, recorder, and RViz...' && echo 'In this bag window, press Space to pause/resume rosbag playback.' && sleep 8 && rosbag play --clock --rate ${BAG_RATE} ${BAG_PATH} --topics ${ALGO_BAG_TOPICS}" Enter
