...
BAG_PATH="${BAG_PATH_OVERRIDE:-${DATASET_DEFAULT_BAG:-/data/UrbanNav-HK_TST-20210517_sensors.bag}}"
BAG_RATE="${BAG_RATE:-0.5}"
GT_YAW_OFFSET_DEG="${GT_YAW_OFFSET_DEG:-0.0}"
GT_PATH="${GT_PATH_OVERRIDE:-${DATASET_DEFAULT_GT:-/data/UrbanNav_TST_GT_raw.txt}}"
...
RESULT_DIR="${RESULTS_ROOT}/per_${PER}"
...
tmux send-keys -t "${BRIDGE_PANE}" "${SETUP} && ${ROS_ENV} && roslaunch localization_benchmark custom_bridge.launch dataset:=${DATASET_LABEL:-urbannav_hk_tst_20210517} source_lidar_topic:=${DATASET_SOURCE_LIDAR_TOPIC:-/velodyne_points} source_imu_topic:=${DATASET_SOURCE_IMU_TOPIC:-/imu/data} source_camera_topic:=${DATASET_SOURCE_CAMERA_TOPIC:-/zed2/camera/right/image_raw} source_left_camera_topic:=${DATASET_SOURCE_LEFT_CAMERA_TOPIC:-/zed2/camera/left/image_raw} use_clock_stamp:=${DATASET_USE_CLOCK_STAMP:-false}" Enter

ADAPTER_PANE="$(tmux split-window -v -t "${TF_PANE}" -P -F "#{pane_id}")"
tmux send-keys -t "${ADAPTER_PANE}" "${SETUP} && ${ROS_ENV} && ${ALGO_ADAPTER_LAUNCH} run_id:=${PER} perturbation_config:=${CONFIG_PATH} native_lidar_topic:=${ALGO_NATIVE_LIDAR_TOPIC} native_lidar_frame_id:=${DATASET_LIDAR_FRAME:-velodyne} native_imu_topic:=${ALGO_NATIVE_IMU_TOPIC} native_imu_frame_id:=${DATASET_IMU_FRAME:-body} native_camera_topic:=${ALGO_NATIVE_CAMERA_TOPIC} camera_frame_id:=${DATASET_CAMERA_FRAME:-camera_right} point_time_scale:=${ALGO_POINT_TIME_SCALE} point_time_field:=${DATASET_POINT_TIME_FIELD:-time} point_time_unit:=${DATASET_POINT_TIME_UNIT:-auto} ${ALGO_ADAPTER_ARGS}" Enter
...
if [ "${DATASET_ID}" = "e2o" ]; then
    ALGO_BAG_TOPICS="${ALGO_BAG_TOPICS:-${DATASET_SOURCE_LIDAR_TOPIC} ${DATASET_SOURCE_IMU_TOPIC}}"
fi
tmux send-keys -t "${BAG_PANE}" "${SETUP} && ${ROS_ENV} && echo 'Waiting 8s for bridge, adapter, algorithm, output selector, recorder, and RViz...' && echo 'Press Space here to pause/resume rosbag.' && sleep 8 && rosbag play --clock --rate ${BAG_RATE} ${BAG_PATH} --topics ${ALGO_BAG_TOPICS:-${DATASET_BAG_TOPICS}}" Enter
...
tmux send-keys -t "${RVIZ_PANE}" "${SETUP} && ${ROS_ENV} && rosrun fast_lio_urbannav ground_truth_path_node.py _ground_truth_path:=${GT_PATH} _topic:=/ground_truth_path _odom_topic:=/ground_truth_odometry _frame_id:=${DATASET_WORLD_FRAME:-camera_init} _publish_rate:=10.0 _yaw_offset_deg:=${GT_YAW_OFFSET_DEG} _publish_full_path:=true & rosrun localization_benchmark bag_clock_marker.py _frame_id:=${DATASET_WORLD_FRAME:-camera_init} & rosrun localization_benchmark path_from_csv.py _csv_path:=${BASELINE_CSV} _topic:=/benchmark/baseline_path _frame_id:=${DATASET_WORLD_FRAME:-camera_init} & rosrun localization_benchmark path_from_csv.py _csv_path:=${RESULT_DIR}/trajectory.csv _topic:=/benchmark/selected_run_path _frame_id:=${DATASET_WORLD_FRAME:-camera_init} & LIBGL_ALWAYS_SOFTWARE=1 MESA_LOADER_DRIVER_OVERRIDE=llvmpipe rviz -d ${RVIZ_CONFIG}" Enter

tmux select-window -t "${SESSION}:5"
echo "[benchmark] dataset=${DATASET_DISPLAY:-UrbanNav} mode=interactive algo=${ALGO_DISPLAY} key=${ALGO_ID} case=per_${PER}"
