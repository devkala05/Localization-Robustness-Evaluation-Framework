...
public:
    ...
    IMUPreintegration()
    {
        ...
        // Publish under the configured project namespace so imageProjection can use
        // odomTopic consistently and avoid collisions with other algorithms.
        pubImuOdometry = nh.advertise<nav_msgs::Odometry> (PROJECT_NAME + "/lidar/odometry/imu", 2000);
    ...
    void odometryHandler(const nav_msgs::Odometry::ConstPtr& odomMsg)
    {
        ...
        double preintDt = imuIntegratorOpt_->deltaTij();
        if (preintDt < 1e-5)
        {
            ROS_WARN_THROTTLE(5.0, "Skipping IMU preintegration update with near-zero dt.");
            return;
        }
        ...
        {
            ROS_WARN("IMU preintegration optimization failed, resetting: %s", e.what());
            graphFactors.resize(0);
            graphValues.clear();
            resetParams();
            return;
