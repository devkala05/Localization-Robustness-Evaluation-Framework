...
class OutputSelector:
    def __init__(self):
        ...
        self.last_pose=None
        self.last_gps_odom=None; self.last_gps_wall=0.0; self.gps_count=0; self.last_warn=0.0
    ...
    def gps_cb(self,msg):
        self.last_gps_odom=msg; self.last_gps_wall=time.monotonic(); self.gps_count += 1
        if self.gps_count == 1:
            rospy.loginfo('[OutputSelector] first GPS-fused odometry received stamp=%.9f frame=%s', msg.header.stamp.to_sec(), msg.header.frame_id)
        ...
        if self.gps_enabled and self.last_gps_odom is not None and time.monotonic()-self.last_gps_wall<=self.gps_timeout:
            ...
            if self.gps_enabled:
                ...
                if now-self.last_warn>10.0:
                    rospy.logwarn('[OutputSelector] GPS enabled but no fresh GPS-fused odometry on %s; publishing local fallback', self.gps_topic)
                    self.last_warn=now
