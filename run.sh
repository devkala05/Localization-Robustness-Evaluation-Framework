...
Modes:
  ...
  fusion_2           LVI-SAM + ORB-SLAM3 fusion.
...
is_known_mode() {
  case "$MODE" in
    fast_livo2|orbslam3|lvisam|fusion|fusion_2|fusion_navigation) return 0 ;;
    *) return 1 ;;
  esac
}
...
needs_orb() {
  is_mode orbslam3 || is_mode fusion || is_mode fusion_2 || is_mode fusion_navigation
}

needs_lvisam() {
  is_mode lvisam || is_mode fusion_2
}

uses_fusion_node() {
  is_mode fusion || is_mode fusion_2
}

default_bag_rate() {
  if [[ -n "${BAG_RATE:-}" ]]; then
    printf '%s\n' "$BAG_RATE"
  elif is_mode fast_livo2 || is_mode lvisam || is_mode fusion_2; then
...
if is_mode fusion_2 && [[ "$PRIMARY_SOURCE" == "fast_livo2" ]]; then
  fail "fusion_2 starts LVI-SAM + ORB-SLAM3 only; use PRIMARY_SOURCE=lvisam or PRIMARY_SOURCE=orbslam3."
fi
...
if uses_fusion_node; then
  start_fusion
elif is_mode fusion_navigation; then
  start_fusion_navigation
else
  start_health_only
fi
