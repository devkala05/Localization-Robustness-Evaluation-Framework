...
void img_callback(const sensor_msgs::ImageConstPtr &img_msg)
{
    ...
    cv::Mat input_img = ptr->image;
    if (input_img.rows != ROW * NUM_OF_CAM || input_img.cols != COL)
    {
        ROS_WARN_THROTTLE(
            5.0,
            "resizing camera image from %dx%d to configured LVI-SAM size %dx%d",
            input_img.cols,
            input_img.rows,
            COL,
            ROW * NUM_OF_CAM);
        cv::resize(input_img, input_img, cv::Size(COL, ROW * NUM_OF_CAM));
    }
    cv::Mat show_img = input_img;
    TicToc t_r;
    for (int i = 0; i < NUM_OF_CAM; i++)
    {
        ROS_DEBUG("processing camera %d", i);
        if (i != 1 || !STEREO_TRACK)
            trackerData[i].readImage(input_img.rowRange(ROW * i, ROW * (i + 1)), cur_img_time);
        else
        {
            if (EQUALIZE)
            {
                cv::Ptr<cv::CLAHE> clahe = cv::createCLAHE();
                clahe->apply(input_img.rowRange(ROW * i, ROW * (i + 1)), trackerData[i].cur_img);
            }
            else
                trackerData[i].cur_img = input_img.rowRange(ROW * i, ROW * (i + 1));
        }
...
int main(int argc, char **argv)
{
    ...
    if(FISHEYE)
    {
        for (int i = 0; i < NUM_OF_CAM; i++)
        {
            ...
            if (trackerData[i].fisheye_mask.rows != ROW || trackerData[i].fisheye_mask.cols != COL)
            {
                ROS_WARN(
                    "resizing fisheye mask from %dx%d to configured LVI-SAM size %dx%d",
                    trackerData[i].fisheye_mask.cols,
                    trackerData[i].fisheye_mask.rows,
                    COL,
                    ROW);
