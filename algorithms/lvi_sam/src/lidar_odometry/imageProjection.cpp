...
public:
    ...
    deskewFlag(0)
    {
        ...
        // In the original LVI-SAM pipeline this used visual/VINS propagated odometry.
        // For the UrbanNav LiDAR-IMU benchmark the visual pipeline is disabled, so
        // use the IMU-preintegration odometry topic from the lidar odometry stack.
        // Without this, mapOptimization gets no translation initial guess and the
        // vehicle can appear stuck near the start even while scans are received.
    ...
    void projectPointCloud()
    {
        ...
        for (int i = 0; i < cloudSize; ++i)
        {
            ...
            thisPoint = deskewPoint(&thisPoint, laserCloudIn->points[i].time * pointTimeScale); // Velodyne: convert point time to seconds
