...
Usage:
  ...
  ./run_rtabmap.sh 0 --eval              # positional shorthand
  ./run_rtabmap.sh --shell

Environment overrides:
  BAG_RATE=0.35                 Rosbag playback speed
  GT_YAW_OFFSET_DEG=0.0          GT yaw offset used by RViz/eval
  SKIP_RUNTIME_BUILD=true        Use RTAB-Map wrapper already built into image
...
if [ -n "${CONTAINER_NAME_ENV}" ]; then
    ...
    CONTAINER_NAME="rtabmap_run_${PER}"
else
    CONTAINER_NAME="rtabmap_run"
fi
...
echo "Starting ${CONTAINER_NAME} mode=${MODE}${PER:+ per_${PER}} algo=RTAB-Map eval=${EVALUATE}"
