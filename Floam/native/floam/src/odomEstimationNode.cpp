...
double initial_z_search_min = 0.0;
double initial_z_search_max = 15.0;
double initial_z_offset = 1.5;
double initial_z_fallback = 0.0;

int findLowestMapZ(const pcl::PointCloud<pcl::PointXYZI>::Ptr& cloud, double x, double y,
                   double min_distance, double max_distance, double& z_min)
{
  int candidates = 0;
...
void initialPoseHandler(geometry_msgs::PoseWithCovarianceStamped msg)
{
  if (relocalize && !is_odom_inited && !gotInitialPose)
  {
    ...
    ROS_INFO_STREAM("Received initial pose from RViz: x=" << initialX << " y=" << initialY
    ...
    if (!pointCloudEdgeBuf.empty() && !pointCloudSurfBuf.empty())
    {
      ...
      if (is_odom_inited == false)
      {
        ...
        if (relocalize)
        {
          ...
          double x = initialX;
          double y = initialY;
          double z = std::isfinite(initialPose.position.z) ? initialPose.position.z : initial_z_fallback;
          double z_min = std::numeric_limits<double>::infinity();
          int z_candidates = findLowestMapZ(pointcloud_surf_in, x, y, initial_z_search_min,
                                            initial_z_search_max, z_min);
          std::string z_source = "extracted map surface";

          if (z_candidates == 0 && map && !map->empty())
          {
            z_candidates = findLowestMapZ(map, x, y, initial_z_search_min, initial_z_search_max, z_min);
            z_source = "raw PCD map";
          }

          if (z_candidates > 0 && std::isfinite(z_min))
          {
            z = z_min + initial_z_offset;
          }
          else if (!std::isfinite(z))
          {
            z = initial_z_fallback;
          }

          if (z_candidates == 0)
          {
            ROS_WARN_STREAM("No map points found within " << initial_z_search_min << "-"
                                                          << initial_z_search_max
                                                          << "m of initial x/y. Using RViz/default z="
                                                          << z << ". Check the clicked position on the map.");
          }
          else
          {
            ROS_INFO_STREAM("Initial z resolved from " << z_source << " with " << z_candidates
                                                       << " candidates: ground_z=" << z_min
                                                       << " offset=" << initial_z_offset
                                                       << " initial_z=" << z);
          }
...
int main(int argc, char** argv)
{
  ...
  nh.getParam("/initial_z_search_min", initial_z_search_min);
  nh.getParam("/initial_z_search_max", initial_z_search_max);
  nh.getParam("/initial_z_offset", initial_z_offset);
  nh.getParam("/initial_z_fallback", initial_z_fallback);

  if (initial_z_search_max < initial_z_search_min)
  {
    ROS_WARN_STREAM("initial_z_search_max is less than initial_z_search_min; swapping values");
    std::swap(initial_z_search_min, initial_z_search_max);
  }
