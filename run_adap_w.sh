...
Usage:
  ...
  ./run_adap_w.sh 0 --eval              # positional shorthand
  ./run_adap_w.sh --shell

Environment overrides:
  BAG_RATE=0.35                 Rosbag playback speed
  GT_YAW_OFFSET_DEG=0.0          GT yaw offset used by RViz/eval
  SKIP_RUNTIME_BUILD=true        Use Adaptive-W LVIO already built into image
...
if [ -n "${CONTAINER_NAME_ENV}" ]; then
    ...
    CONTAINER_NAME="adap_w_run_${PER}"
else
    CONTAINER_NAME="adap_w_run"
fi
...
echo "Starting ${CONTAINER_NAME} mode=${MODE}${PER:+ per_${PER}} algo=Adaptive-W LVIO eval=${EVALUATE}"
