...
Among systems without a configured localization failure, **FLOAM** has the smallest worst-case relative 3D-RMSE change (-1.2%). **FAST-LIVO2** has the lowest clean-baseline 3D RMSE (0.521 m), but its worst perturbation increase is +4.1%.
...
Smallest score change among operational systems: **FAST-LIO2** (-7.1%). Largest increase: **ORB-SLAM3** (+2.8%).

## Fog

Smallest score change among operational systems: **FAST-LIVO2** (-3.9%). Largest increase: **ORB-SLAM3** (+38.0%).

## Sensor Degradation

Smallest score change among operational systems: **FLOAM** (-1.2%). Largest increase: **FAST-LIVO2** (+4.1%).

## Sensor dependency and recovery

- **FAST-LIVO2** (LiDAR-camera-IMU): Rain: 0.011 s, Fog: 0.080 s, Sensor Degradation: 0.009 s.
...
- **ORB-SLAM3** (visual RGB-D): Rain: 0.054 s, Fog: 11.755 s, Sensor Degradation: 0.095 s.
