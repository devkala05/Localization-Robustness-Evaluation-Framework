#!/usr/bin/env python3
"""
bag_player.py
=============
Controlled rosbag playback for the UrbanNav-HK pipeline.

Features:
  - Plays only the topics needed by the wrapper (LiDAR, IMU, camera)
  - Supports rate control (default 1.0x)
  - Adds a startup delay to let the wrapper node initialise
  - Optionally loops the bag
  - Publishes /clock when using simulated time

Usage (via rosrun):
    rosrun fast_lio_urbannav bag_player.py \
        _bag_path:=/data/UrbanNav-HK_TST-20210517_sensors.bag \
        _rate:=1.0 \
        _start_delay:=3.0 \
        _loop:=false

Usage (direct):
    python3 bag_player.py --bag /data/bag.bag --rate 1.0
"""

import os
import sys
import time
import subprocess
import argparse
import rospy


# ─── Topics to extract from the rosbag ───────────────────────────────────────
# Only the three sensor streams; no GPS, no ground truth
ALLOWED_TOPICS = [
    "/velodyne_points",        # Centre Velodyne LiDAR  → wrapper → /cloud_registered_raw
    "/imu/data",               # Xsens IMU              → wrapper → /livox/imu
    "/zed2/camera/right/image_raw",  # ZED2 right camera → wrapper → /camera/right/image_raw
]


def play_bag(bag_path: str,
             rate: float = 1.0,
             start_delay: float = 3.0,
             loop: bool = False,
             start_offset: float = 0.0,
             use_sim_time: bool = True):
    """Launch rosbag play as a subprocess."""

    if not os.path.isfile(bag_path):
        rospy.logerr(f"[BagPlayer] Bag not found: {bag_path}")
        sys.exit(1)

    rospy.loginfo(f"[BagPlayer] Waiting {start_delay:.1f}s for wrapper to initialise …")
    time.sleep(start_delay)

    cmd = [
        "rosbag", "play",
        bag_path,
        "--rate", str(rate),
        "--start", str(start_offset),
    ]

    if use_sim_time:
        cmd.append("--clock")

    if loop:
        cmd.append("--loop")

    # Only play the topics we need
    cmd += ["--topics"] + ALLOWED_TOPICS

    rospy.loginfo("[BagPlayer] Running: %s", " ".join(cmd))
    rospy.loginfo(
        "[BagPlayer] Playing topics:\n" +
        "".join(f"  {t}\n" for t in ALLOWED_TOPICS)
    )

    try:
        proc = subprocess.run(cmd, check=True)
    except subprocess.CalledProcessError as e:
        rospy.logerr(f"[BagPlayer] rosbag play exited with code {e.returncode}")
        sys.exit(e.returncode)
    except KeyboardInterrupt:
        rospy.loginfo("[BagPlayer] Interrupted by user.")


def main():
    rospy.init_node("bag_player", anonymous=True)

    bag_path     = rospy.get_param("~bag_path",     "/data/UrbanNav-HK_TST-20210517_sensors.bag")
    rate         = float(rospy.get_param("~rate",          1.0))
    start_delay  = float(rospy.get_param("~start_delay",   3.0))
    loop         = rospy.get_param("~loop",          False)
    start_offset = float(rospy.get_param("~start_offset",  0.0))
    use_sim_time = rospy.get_param("~use_sim_time",  True)

    play_bag(
        bag_path=bag_path,
        rate=rate,
        start_delay=start_delay,
        loop=loop,
        start_offset=start_offset,
        use_sim_time=use_sim_time,
    )


if __name__ == "__main__":
    # Allow running directly without rosrun for quick testing
    if "--bag" in sys.argv:
        parser = argparse.ArgumentParser()
        parser.add_argument("--bag",          required=True)
        parser.add_argument("--rate",         type=float, default=1.0)
        parser.add_argument("--start-delay",  type=float, default=3.0)
        parser.add_argument("--loop",         action="store_true")
        parser.add_argument("--start-offset", type=float, default=0.0)
        parser.add_argument("--no-sim-time",  action="store_true")
        args = parser.parse_args()
        play_bag(
            bag_path=args.bag,
            rate=args.rate,
            start_delay=args.start_delay,
            loop=args.loop,
            start_offset=args.start_offset,
            use_sim_time=not args.no_sim_time,
        )
    else:
        main()
