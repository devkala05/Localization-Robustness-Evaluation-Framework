...
class GnssSolutionReplayer:
    def __init__(self):
        ...
        self.rows=[]; self.stamps=[]; self.index=-1
        ...
        rospy.loginfo('[GNSSReplayer] file=%s rows=%d output=%s time_offset=%.3f', self.csv_path or 'none', len(self.rows), self.output_topic, self.time_offset_sec)
    ...
    def load_csv(self,path):
        with open(path,'r',encoding='utf-8') as f:
            ...
            for r in reader:
                try:
                    stamp=float(r.get('stamp') or r.get('time') or r.get('timestamp')) + self.time_offset_sec
