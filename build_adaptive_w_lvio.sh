...
for path in \
    "${SCRIPT_DIR}/docker/adaptive_w_lvio/Dockerfile" \
...
echo "[build] algorithm=Adaptive-W LVIO image=${IMAGE_NAME}"
docker build ${CACHE_FLAG} --network host \
    -t "${IMAGE_NAME}" \
    -f "${SCRIPT_DIR}/docker/adaptive_w_lvio/Dockerfile" \
    "${SCRIPT_DIR}"

echo "[build] complete algorithm=Adaptive-W LVIO"
