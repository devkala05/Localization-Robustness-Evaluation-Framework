...
class UrbanNavCustomBridge:
    def __init__(self):
        ...
        self.counts = {"lidar": 0, "imu": 0, "camera_right": 0}
    ...
    def camera_cb(self, msg):
        ...
        out.header = msg.header
        out.dataset = self.dataset
        out.sensor_name = "zed2_right"
        out.source_topic = self.camera_topic
        out.perturbation_label = "none"
        out.image = msg
    ...
    def log_rates(self, _event):
        with self.lock:
            rospy.loginfo(
                "[UrbanNavCustomBridge] totals lidar=%d imu=%d camera=%d",
                self.counts["lidar"],
                self.counts["imu"],
