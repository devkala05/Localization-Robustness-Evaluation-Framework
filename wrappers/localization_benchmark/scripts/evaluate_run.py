...
def load_ground_truth_csv(path, yaw_offset_deg=0.0):
    rows = []
    ...
    with open(path, newline="", encoding="utf-8", errors="ignore") as handle:
        ...
        if not reader.fieldnames or "timestamp_s" not in reader.fieldnames:
            return []
        for row in reader:
            stamp = finite_float(row.get("timestamp_s"), None)
            if stamp is None:
                continue
            yaw = row.get("yaw_rad")
            if yaw in (None, "", "nan"):
                yaw = quaternion_yaw(
                    finite_float(row.get("qx")),
                    finite_float(row.get("qy")),
                    finite_float(row.get("qz")),
                    finite_float(row.get("qw"), 1.0),
                )
            else:
                yaw = finite_float(yaw)
            x, y = rotate_xy(finite_float(row.get("x_m")), finite_float(row.get("y_m")), yaw_offset_rad)
            rows.append({
                "stamp": stamp,
                "x": x,
                "y": y,
                "z": finite_float(row.get("z_m")),
                "yaw": angle_diff(yaw + yaw_offset_rad, 0.0),
            })
...
def load_ground_truth(path, yaw_offset_deg=0.0):
    csv_rows = load_ground_truth_csv(path, yaw_offset_deg)
    if csv_rows:
        return csv_rows
    raw = []
    ...
    if not raw:
        return []
    raw.sort(key=lambda row: row["stamp"])
    yaw_offset_rad = math.radians(yaw_offset_deg)
    if raw[0].get("format") == "tum":
        raw = [row for row in raw if row.get("format") == "tum"]
        ref = raw[0]
        ...
        for row in raw:
            ...
            out.append({
                ...
                "y": y,
                "z": row["z"] - ref["z"],
                "yaw": angle_diff(row["yaw"] - ref["yaw"] + yaw_offset_rad, 0.0),
            })
        return out
    raw = [row for row in raw if row.get("format") == "urbannav"]
    ref = raw[0]
