...
def load_ground_truth(path: str, yaw_offset_deg: float) -> List[dict]:
    ...
    if path.lower().endswith(".csv"):
        ...
        with open(path, "r", encoding="utf-8", errors="ignore") as handle:
            ...
            if {"timestamp", "pos_x", "pos_y"}.issubset(fields) or {"x_m", "y_m"}.issubset(fields):
                for row in reader:
                    try:
...
def main() -> int:
    ...
    parser.add_argument("--dataset", default=os.environ.get("DATASET", ""), help="Dataset defaults: urbannav or e2o")
    ...
    dataset = DATASET_ALIASES.get(str(args.dataset or "").strip().lower())
    if dataset == "e2o":
        if args.results_root == "/data/results":
            args.results_root = "/data/results/e2o"
        if args.gt == "/data/UrbanNav_TST_GT_raw.txt":
