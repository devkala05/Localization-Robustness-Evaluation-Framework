...
When the native process is restarted in the middle of a fault run, FAST-LIVO2
starts a fresh local map frame. The wrapper can seed that fresh native frame
from the current fused pose so the exported trajectory resumes at the active
localization pose instead of jumping back to an arbitrary local origin.
"""
import copy
import math
import time
...
class OdometryAliasNode:
    def __init__(self):
        ...
        rospy.loginfo(
            "[FASTLIVO2 OdomAlias] preferred=%s fallback=%s -> %s rebase=%s anchor=%s",
    ...
    def anchor_cb(self, msg):
        try:
            ...
            rospy.logwarn_throttle(2.0, "[FASTLIVO2 OdomAlias] rejected anchor pose: %s", exc)
    ...
    def rebased_message(self, msg):
        try:
            ...
            rospy.logwarn_throttle(2.0, "[FASTLIVO2 OdomAlias] dropping invalid native pose: %s", exc)
            return None
        if self.output_from_native is None:
            if self.rebase_on_start and self.latest_anchor_T is not None:
                ...
                if anchor_age <= self.anchor_max_age_sec:
                    ...
                    rospy.logwarn(
                        "[FASTLIVO2 OdomAlias] rebased native startup frame to %s using %s",
            ...
            if self.output_from_native is None:
                ...
                if (self.rebase_on_start and elapsed < self.anchor_giveup_sec and
                    ...
                    rospy.logwarn_throttle(
                        2.0,
                        "[FASTLIVO2 OdomAlias] waiting for fresh startup anchor from %s; refusing native frame",
                        self.anchor_pose_topic,
                    )
                    return None
                self.output_from_native = np.eye(4, dtype=float)
                rospy.logwarn(
                    "[FASTLIVO2 OdomAlias] no fresh startup anchor within %.1fs; publishing native frame",
