...
Notation from extrinsic.yaml:
    X_T_Y  means  "pose of Y expressed in X's frame"
    i.e.  p_X = X_T_Y * p_Y
So body→sensor = inv(SENSOR_T_IMU).
...
# Convention: SENSOR_T_IMU means p_sensor = M * p_imu
# → body→sensor transform used in TF = inv(M)
...
def build_static_transforms():
    ...
    # velodyne → velodyne_left / velodyne_right  (auxiliary LiDARs)
