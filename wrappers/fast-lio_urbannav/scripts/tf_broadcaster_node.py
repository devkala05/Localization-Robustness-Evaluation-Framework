...
FAST-LIO2 and LVI-SAM can use the same calibration without TF conflicts:
  ...
  LVI-SAM mode:
    ...
    publish_base_link_body:=true  # connects LVI base_link to UrbanNav body/sensors
...
# Convention used by the reference FAST-LIVO2 wrapper:
# SENSOR_T_IMU means p_sensor = SENSOR_T_IMU * p_imu.
# Therefore body(IMU) -> sensor TF = inv(SENSOR_T_IMU).
...
def build_static_transforms():
    ...
    tfs.append(make_stamped_tf(invert_rigid(CENTER_LIDAR_T_IMU), "body", "velodyne"))
    tfs.append(make_stamped_tf(invert_rigid(LEFT_LIDAR_T_CENTER), "velodyne", "velodyne_left"))
    tfs.append(make_stamped_tf(invert_rigid(RIGHT_LIDAR_T_CENTER), "velodyne", "velodyne_right"))
    tfs.append(make_stamped_tf(invert_rigid(LEFT_CAMERA_T_IMU), "body", "camera_left"))
    tfs.append(make_stamped_tf(invert_rigid(RIGHT_CAMERA_T_IMU), "body", "camera_right"))
    ...
    tfs.append(make_stamped_tf(invert_rigid(ANTENNA_T_IMU), "body", "gnss_antenna"))

    identity = np.eye(4)
    if publish_map_odom:
        tfs.append(make_stamped_tf(identity, "map", "odom"))
    if publish_camera_init_map:
        tfs.append(make_stamped_tf(identity, "camera_init", "map"))
    if publish_base_link_body:
        tfs.append(make_stamped_tf(identity, "base_link", "body"))
        tfs.append(make_stamped_tf(identity, "lidar_link", "body"))
