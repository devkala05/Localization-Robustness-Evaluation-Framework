...
def parse_csv_ground_truth(path: str) -> List[dict]:
    ...
    def yaw_from_row(row: dict) -> float:
        raw = value(row, "yaw_rad")
        if raw not in (None, ""):
            yaw = float(raw)
            if math.isfinite(yaw):
                return yaw
        qx = float(value(row, "qx", default=0.0) or 0.0)
        qy = float(value(row, "qy", default=0.0) or 0.0)
        qz = float(value(row, "qz", default=0.0) or 0.0)
        qw = float(value(row, "qw", default=1.0) or 1.0)
    ...
    with open(path, "r", encoding="utf-8", errors="ignore") as handle:
        ...
        for row in reader:
            try:
                stamp_raw = value(row, "timestamp_s", "timestamp")
                stamp = float(stamp_raw) if stamp_raw not in (None, "") else float(row["timestamp_ns"]) * 1.0e-9
                yaw = yaw_from_row(row)
                samples.append({
                    "stamp": stamp,
                    "x": float(value(row, "x_m", "pos_x")),
                    "y": float(value(row, "y_m", "pos_y")),
                    "z": float(value(row, "z_m", "pos_z", default=0.0) or 0.0),
                    "yaw": yaw,
                    "qx": float(row.get("qx", 0.0) or 0.0),
                    "qy": float(row.get("qy", 0.0) or 0.0),
                    "qz": float(row.get("qz", math.sin(0.5 * yaw)) or math.sin(0.5 * yaw)),
                    "qw": float(row.get("qw", math.cos(0.5 * yaw)) or math.cos(0.5 * yaw)),
                })
...
def parse_ground_truth(path: str) -> List[dict]:
    csv_samples = parse_csv_ground_truth(path) if path.lower().endswith(".csv") else []
    if csv_samples:
        return csv_samples
    samples = []
    with open(path, "r", encoding="utf-8", errors="ignore") as handle:
...
def quaternion_from_sample(sample: dict) -> Quaternion:
    q = Quaternion()
    if all(k in sample for k in ("qx", "qy", "qz", "qw")):
        q.x = float(sample["qx"])
        q.y = float(sample["qy"])
        q.z = float(sample["qz"])
        q.w = float(sample["qw"])
        return q
    return yaw_quaternion(float(sample.get("yaw", 0.0)))
...
def build_path(samples: List[dict], frame_id: str, rotate_to_initial_heading: bool, yaw_offset_deg: float, z_offset: float) -> Path:
    if samples and "x" in samples[0]:
        yaw_offset_rad = math.radians(yaw_offset_deg)
        ref = samples[0]
        ref_x = float(ref["x"])
        ref_y = float(ref["y"])
        ref_z = float(ref.get("z", 0.0))
        ref_yaw = float(ref.get("yaw", 0.0))
        path_msg = Path()
        path_msg.header.frame_id = frame_id
        for sample in samples:
            x, y = rotate_xy(float(sample["x"]) - ref_x, float(sample["y"]) - ref_y, yaw_offset_rad)
            pose = PoseStamped()
            pose.header.frame_id = frame_id
            pose.header.stamp = rospy.Time.from_sec(float(sample["stamp"]))
            pose.pose.position.x = x
            pose.pose.position.y = y
            pose.pose.position.z = float(sample.get("z", 0.0)) - ref_z + z_offset
            if "yaw" in sample:
                pose.pose.orientation = yaw_quaternion(
                    normalize_angle(float(sample["yaw"]) - ref_yaw + yaw_offset_rad)
                )
            else:
                pose.pose.orientation = quaternion_from_sample(sample)
