...
def parse_csv_samples(path, yaw_offset_deg=0.0):
    rows = []
    ...
    with open(path, newline="", encoding="utf-8", errors="ignore") as handle:
        ...
        for row in reader:
            ...
            x, y = rotate_xy(finite_float(row.get("x_m")), finite_float(row.get("y_m")), offset)
            rows.append({
                "stamp": stamp,
                "x": x,
                "y": y,
...
def parse_local_samples(path, yaw_offset_deg=0.0):
    csv_samples = parse_csv_samples(path, yaw_offset_deg)
    if csv_samples:
        return csv_samples
    raw = []
...
def main():
    ...
    if not samples:
        rospy.logerr("[GroundTruthPath] No valid INS/TUM/CSV samples in %s",gt_path); return
