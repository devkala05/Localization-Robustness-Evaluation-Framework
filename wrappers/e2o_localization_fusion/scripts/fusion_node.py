...
class LocalizationFusion:
    ...
    def evaluate_state_machine(self, now: float) -> None:
        ...
        if active == self.secondary:
            ...
            if primary_ready and dwell_ok and (recovery_consistent or not self.require_recovery_consistency):
                reason = ("primary_recovered_and_consistent" if recovery_consistent
                          else "primary_recovered_continuity_aligned")
    ...
    def timer_cb(self, _event) -> None:
        with self.lock:
            ...
            navigation_ok = (
                ...
                bool(self.sources[self.active_source].health.get("healthy", False)) and
                not degraded
            )
